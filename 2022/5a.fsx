...
let popFrom index stacks =
  List.mapi (fun i stack -> if i = index then List.tail stack else stack) stacks

let pushTo index value stacks =
  List.mapi (fun i stack -> if i = index then value::stack else stack) stacks


let move (stacks : char list list) from dest =
  let toMove = List.nth stacks from |> List.head
  let result = popFrom from stacks |> pushTo dest toMove
  printfn "%O" result
  result
...
let processMove (stacks : char list list) line =
  ...
  let result = Seq.fold (fun s x -> move s from dest) stacks { 1 .. count }
  printfn "ok"
  result
