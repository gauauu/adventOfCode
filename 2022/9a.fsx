...
let nextState state command =
    let (head, tail, (positions: Set<int * int>)) = state
    let newHead = applyCommand head command
    let newTail = chaseHead newHead tail
    (newHead, newTail, positions.Add(newTail))


let result = List.fold nextState state commands

let (_,_,final) = result
Set.count final
