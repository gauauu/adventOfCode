...
# hcl:#623a2f
#
# eyr:2029 ecl:blu cid:129 byr:1920
...
def is_valid?(p)
  reqd = {
    "byr" => ->(x) {x =~ /\d\d\d\d/ and x.to_i.between? 1920, 2002 },
    "iyr" => ->(x) {x =~ /\d\d\d\d/ and x.to_i.between? 2010, 2020 },
    "eyr"=> ->(x) {x =~ /\d\d\d\d/ and x.to_i.between? 2020, 2030 },
    "hgt" => ->(x) {
      if x =~ /(\d+)(\w+)/
        if $2 == "cm"
          return $1.to_i.between? 150, 193
        elsif $2 == "in"
          return $1.to_i.between? 59, 76
        else
          return false
        end
      end
  ...
  groups = p.split
  fields = groups.map { |g| g.chomp.split(":")}.to_h

  reqd.map do |key,validator|
    val = fields[key]
    if val.nil?
      return false
    end

    field_ok = validator.call(val)
    field_ok
...
valid = passports.select { |p|  is_valid? p }
