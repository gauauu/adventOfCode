...
rules = lines.map do |l|
  parent, contained = l.chomp(".").chomp.split(" bags contain ")
  children = contained.split(", ")
  if children[0] == "no other bags"
    children_rules = []
  else
    children_rules = children.map do |c|
      matches = /(\d+) ([ a-z]+) bag.*/.match(c)
      [matches[1], matches[2]]
    end
  end
  [parent,children_rules]
end.to_h


def can_hold_bag(target, bag, rules)
  bag_children = rules[bag]
  #if can hold directly
  if bag_children.any? { |child| child[1] == target }
    return true
  end

  # or if children rules can hold bag
  if bag_children.any? { |child| can_hold_bag(target, child[1], rules)}
    return true
  end
...
def bags_held(bag_color, rules)
  bag_children = rules[bag_color]

  num_direct_children = bag_children.map{|num, color| num.to_i}.sum

  total = num_direct_children + bag_children.map {|num,child_color| bags_held(child_color, rules) * num.to_i}.sum
  total
end


p rules.select {|color,children| can_hold_bag("shiny gold", color, rules)}.keys.length

p bags_held("shiny gold", rules)
